...
<p>' . sprintf(_("pgAdmin packages are available for <a href=\"%s\">Debian</a> stable, testing and unstable distributions. These packages are also well-suited for <a href=\"%s\">Knoppix</a> and other Debian based distributions"), "http://www.debian.org/", "http://www.knoppix.net/") . '</p>
...
<p>' . _("pgAdmin 1.4.3 is available directly from Debian. You can install pgAdmin just as you would do with any other Debian package (apt-get install pgadmin3).") . '</p>

<p>' . sprintf(_("If you need a more up to date version of pgAdmin, an APT repository is provided to ease installation. Before you start installation, please choose a mirror from the list of <a href=\"%s\">PostgreSQL mirrors</a>. Click one of the flags, and copy the URL you are redirected to. On each mirror, the packages are located in: <i>/pgadmin3/release/debian</i>"), "http://wwwmaster.postgresql.org/download/mirrors-ftp") . '</p>
...
<p>' . _("pgAdmin is published on the official Debian Unstable repository. However, from time to time unstable may become broken, you can try to check for packages from our mirrors as described above for Debian/testing. You only need to change testing to unstable in the sources.list lines.") . '</p>
