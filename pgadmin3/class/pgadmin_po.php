...
class po_status {
  ...
  function addLanguage ($_locale, $_language, $_translator_name, $_translator_email, $_po_name, $_po_path, $_pot_name, $_pot_path) {
    ...
    $this->_content['translator_email'][$this->_size]= $_translator_email;
		...
		$this->_content['pot_name'][$this->_size]   = $_pot_name;
		$this->_content['pot_path'][$this->_size]  = $_pot_path;
    $_pot_stat = array();
    $_pot_stat = $this->getStatistics ($this->_cvsroot . "/" . $_pot_path);
    $this->_content['pot_total'][$this->_size] = $_pot_stat['total'];

    $this->_content['po_name'][$this->_size]   = $_po_name;
    $this->_content['po_path'][$this->_size]   = $_po_path;
    $_po_stat = array();
		$_po_stat = $this->getStatistics ($this->_cvsroot . "/" . $_po_path);
  ...
  function display ($_object) {
    ...
    $_result = $_result . "<th><font color=black> ". _("Download Template") ."</font></th>";
    $_result = $_result . "<th><font color=black> ". _("Download Language") ."</font></th>";
    ...
    for ($_count = 0; $_count < $this->_size; $_count++) {
      ...
      $_pot_name  = $this->_content['pot_name'][$_count];
      $_pot_total = $this->_content['pot_total'][$_count];
      $_pot_path  =  "../" .str_replace ($this->_webroot ."/" , "", $this->_cvsroot . "/". $this->_content['pot_path'][$_count]);

      $_po_name       = $this->_content['po_name'][$_count];
      ...
      $_po_status     = $this->_content['po_status'][$_count]."%";
			$_po_path       =  "../" .str_replace ($this->_webroot ."/", "", $this->_cvsroot . "/" . $this->_content['po_path'][$_count]);
      ...
      "<td><a href='$_pot_path'>$_pot_name</a></td>" .
