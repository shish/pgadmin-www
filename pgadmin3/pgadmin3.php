...
require("globals.php");
  ...
  $rows = pg_numrows($rs);
  for ($x = 0;$x < $rows;$x++) {
    $QuickLinks = $QuickLinks . "        &nbsp;&nbsp;<a href=\"pgadmin3.php?ContentID=" . pg_result($rs, $x, "id") . "\">" . pg_result($rs, $x, "quicklink") . "</a><br>\n";
  }
?>
...
<table border=0 cellspacing=3 width=720>
  ...
  <tr>
    <td align="left" valign="middle" width=450 height=30 bgcolor="<?php echo("$LightBlue"); ?>">
      <small>pgAdmin II is Copyright &copy; 2001 - 2003, The <a href="pgadmin3.php?ContentID=3">pgAdmin
    ...
    <td align="left" valign="top" colspan=1 rowspan=1 width=120 height=60 bgcolor="<?php echo("$LightBlue"); ?>">
      <center>
        <small>
          ...
          <form action="pgadmin3.php?ContentID=<?php echo("$doc[id]"); ?>" method="post">
      ...
      <small>
        ...
        &nbsp;&nbsp;<a href="/snapshots/">Snapshots</a><br><br>
        <?php echo("$QuickLinks"); ?>
      </small>
      <br>
    </td>
...
<?php
  ...
  if ($doc[idoc] == "t") {
    ...
    echo ("  <form action=\"pgadmin3.php?ContentID=$doc[id]\" method=\"post\">\n");
